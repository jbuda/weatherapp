...
class ForecastLocalDatasourceImpl implements ForecastLocalDatasource {
  ...
  Future<List<ForecastModel>> getLastFiveDayForecast() {
    ...
    if (jsonString != null) {
      ...
      return Future.value(decoded.map((dynamic item) => ForecastModel.fromJson(item as Map<String, dynamic>)).toList());
    } else {
      throw CacheException();
    }
  }
  ...
  @override
  Future<void> cacheFiveDayForecast(List<ForecastModel> fiveDayForecastToCache) {
    final encoded = fiveDayForecastToCache.map((ForecastModel item) => item.toJson()).toList();

    return sharedPreferences.setString(
        cachedFiveDayForecastKey,
        json.encode(encoded)
    );
  }
}